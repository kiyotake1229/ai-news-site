...
export function NewsList({ initialNews }: NewsListProps) {
  ...
  }, [baseNews, selectedSource, searchQuery, sortType]);
  ...
  const translateNews = useCallback(async (lang: Language, newsToTranslate: (NewsItem | ScoredNewsItem)[]) => {
  ...
  const getDisplayNews = useCallback((news: NewsItem | ScoredNewsItem) => {
        ...
        >
          ...
          {favorites.length > 0 && (
            <span className={`px-2 py-0.5 rounded-full text-xs ${
              activeTab === 'favorites' ? 'bg-white/20' : 'bg-pink-100 dark:bg-pink-900/30 text-pink-600 dark:text-pink-400'
            }`}>
              {favorites.length}
            </span>
          )}
