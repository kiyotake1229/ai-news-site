...
- **basePath**: 本番環境では `/ai-news-site`（GitHub Pagesのリポジトリ名に合わせる）、ローカルでは空
- **RSSフェッチ**: `page.tsx` がクライアントコンポーネント (`'use client'`)。以下の順でCORSプロキシにフォールバック:
...
- **翻訳**: [MyMemory API](https://mymemory.translated.net/) を利用（無料枠: 1日5000文字）。`src/lib/translate.ts` で5件並列処理
...
`src/lib/feeds.ts` の `RSS_FEEDS` 配列にオブジェクトを追加するだけ:

```typescript
{ name: '表示名', url: 'RSSフィードURL', category: 'カテゴリ名' }
```

カテゴリは `日本語` / `公式` / `MCP` / `海外ニュース` / `コミュニティ` / `YouTube` のいずれか。
新カテゴリを追加する場合は `src/components/SourceSettings.tsx` の `CATEGORY_ORDER` と `CATEGORY_ICONS` にも追記する。

## localStorageキー一覧

デバッグ時はブラウザの DevTools → Application → Local Storage で確認:

| キー | 内容 | 管理ファイル |
|------|------|------------|
| `ai-news-cache` | RSSフェッチ結果のキャッシュ（1時間有効） | `src/lib/cache.ts` |
| `ai-news-read` | 既読記事のURLセット（最大500件） | `src/lib/readHistory.ts` |
| `ai-news-favorites` | お気に入り記事（最大1000件） | `src/lib/favorites.ts` |
| `ai-news-disabled-sources` | 非表示にしたソース名の配列 | `src/lib/sourceSettings.ts` |

## 注意事項

- RSSフェッチは全てクライアントサイドで行う（静的サイトのためサーバーサイドAPIルートなし）
- CORSプロキシに依存しているため、全プロキシが落ちると全フィードが取得不可になる
- 各フィードから最大10件取得し、全件を日付降順でソート
- 翻訳はMyMemory APIの無料枠（1日5000文字）に依存。大量翻訳時は制限に注意

## 過去に踏んだバグ（再現防止）

### React StrictMode + `useRef` でページロードが無限スピナーになる

`useEffect` 内で `fetchingRef = useRef(false)` をガードに使うパターンは **使ってはいけない**。

React StrictMode は開発中にエフェクトを2回実行する。1回目で `fetchingRef.current = true` にセットされ、
アンマウント後も `ref` の値が残るため、2回目のエフェクトでは早期リターンし `setLoading(false)` が
呼ばれなくなる → 無限ローディングになる。

**解決策**: `useEffect` 内で直接 async 関数を定義して `.then()` で呼ぶ、`[]` 依存配列のシンプルなパターンを使う。
